...
import { ChevronRight, Loader2, Scale, TrendingDown, TrendingUp, Truck, Users } from "lucide-react"
...
export default function ControlToneladas() {
  ...
  const [desde, setDesde] = useState(() => iso(primerDiaMes(new Date())))
  const [hasta, setHasta] = useState(() => iso(ultimoDiaMes(new Date())))
  ...
  const [totales, setTotales] = useState({ totalToneladas: 0, totalTrabajadores: 0 })
  ...
  const consultar = async () => {
    ...
    if (r.success && r.data) {
      ...
      setTotales({ totalToneladas: r.data.totalToneladas, totalTrabajadores: r.data.totalTrabajadores })
      setExpand(new Set())
  ...
  const aplicarPreset = (preset: "hoy" | "mes" | "mesAnterior" | "trimestre" | "anio") => {
    ...
    if (preset === "hoy") {
      ...
      setDesde(iso(primerDiaMes(hoy)))
      setHasta(iso(ultimoDiaMes(hoy)))
    } else if (preset === "mesAnterior") {
      const anterior = new Date(hoy.getFullYear(), hoy.getMonth() - 1, 1)
      setDesde(iso(primerDiaMes(anterior)))
      setHasta(iso(ultimoDiaMes(anterior)))
  ...
  // KPIs y "menor tonelaje" / "más eficiente" siempre reflejan a TODO el
  // equipo del proyecto en el periodo — el filtro de trabajador solo acota
  // la tabla, para no perder el contexto de equipo al revisar a una persona.
  const menorTonelaje = trabajadores[0] // ya viene ordenado ascendente
  const masEficiente = trabajadores.reduce<TrabajadorToneladas | null>(
    (mejor, t) => (t.metaDia > 0 && (!mejor || t.pctCumplimiento > mejor.pctCumplimiento) ? t : mejor),
    null,
  )
          ...
          {cargado && (
            <>
              <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                <Kpi label="Trabajadores con movimiento" value={String(totales.totalTrabajadores)} />
                <Kpi label="Toneladas del periodo" value={`${t2(totales.totalToneladas)} t`} />
                <Kpi
                  label="Menor tonelaje"
                  value={menorTonelaje ? t2(menorTonelaje.tonAcumulada) + " t" : "—"}
                  hint={menorTonelaje?.persona}
                  tone="down"
                />
                <Kpi
                  label="Más eficiente"
                  value={masEficiente ? `${masEficiente.pctCumplimiento}%` : "—"}
                  hint={masEficiente ? `${masEficiente.persona} vs. meta` : "Sin meta configurada"}
                  tone="up"
                />
              </div>
              ...
              <Card>
                ...
                <CardContent className="overflow-x-auto">
                                ...
                                <TableRow className="bg-muted/20 hover:bg-muted/20">
                                  <TableCell colSpan={8} className="px-2 py-2">
                                    <div className="grid gap-3 md:grid-cols-[1fr_auto]">
                                      ...
                                      <div className="max-h-72 min-w-[180px] overflow-auto rounded border bg-background p-2">
                                        <p className="mb-1.5 text-xs font-semibold">Toneladas por día (historial del periodo)</p>
                                        <div className="space-y-1">
                                          {t.tonPorDia.map((d) => (
                                            <div key={d.fecha} className="flex items-center justify-between text-xs">
                                              <span className="text-muted-foreground">{d.fecha.slice(5)}</span>
                                              <span className="font-medium tabular-nums">{t2(d.toneladas)}</span>
                                            </div>
                                          ))}
                                        </div>
                                        {t.vehiculos.length > 0 && (
                                          <>
                                            <p className="mb-1.5 mt-3 text-xs font-semibold">Vehículos atendidos</p>
                                            <div className="flex flex-wrap gap-1">
                                              {t.vehiculos.map((v) => (
                  ...
                  <p className="mt-2 text-xs text-muted-foreground">
                    ...
                    real de toneladas/día contra la Meta configurada en Financiera › Tarifas › Metas (— si el
                    proyecto no tiene meta). "Inactivo hoy" marca a quien ya no está en Head Count (se retiró después
                    ...
                    completo día a día. Toca un trabajador para ver el detalle de sus órdenes y vehículos atendidos.
                  </p>
                </CardContent>
              </Card>
            </>
