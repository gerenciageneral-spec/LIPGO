...
console.log("[v0] selectedEmpresaId:", selectedEmpresaId)
  ...
  return (
    <div className="space-y-6 p-6">
      <Card>
        <CardHeader>
          <CardTitle>Generar Orden de Descargue</CardTitle>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
                  ...
                  {vehicleAppointments.length === 0 ? (
                    ...
                    vehicleAppointments.map((appointment) => (
                      <SelectItem key={appointment.id} value={appointment.id.toString()}>
                        {appointment.placa} - {new Date(appointment.fechallegada).toLocaleDateString()}
                      </SelectItem>
                    ))
                  )}
                </SelectContent>
            ...
            <div className="space-y-2">
              <Label htmlFor="peso-bascula">Peso Báscula (toneladas)</Label>
              <Input
                id="peso-bascula"
                type="number"
                step="0.001"
            ...
            {orderData.lineas.length > 0 && (
              <div className="flex justify-end pt-4">
                <div className="text-right">
                  ...
                  <p className="text-2xl font-bold">{getTotalPeso().toFixed(2)} kg</p>
                </div>
              </div>
            )}
          </div>
